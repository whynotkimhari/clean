...
IsEvenDecimal x y = (toInt((x + y) * 10.0) rem 10) rem 2 == 0
...
isPerfectSquare x = toInt(sqrt(toReal(x))) * toInt(sqrt(toReal(x))) == x
...
transform x = toString (y) +++ " year " +++ toString (z / 7) +++ " week " +++ toString (z - (z / 7) * 7) +++ " days"
where y = x / 365
      z = x - y * 365
