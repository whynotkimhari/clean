...
// exNode function (extract Node) - for getting node's value
exNode :: (Tree Int) -> Int
exNode Leaf = 0
exNode (Node x l r) = x

// f10 - solve the problem
f10 :: (Tree Int) Int -> Int
f10 Leaf _ = 0
f10 (Node x l r) n
| x == n = (exNode l + exNode r) + (f10 l n) + (f10 r n)
= (f10 l n) + (f10 r n)
...
| n == x = (Node 0 (replace n l) (replace n r))
...
isBST :: (BST Int) -> Bool
isBST tree = list == sort list
where list = treeToList tree
