...
sqrs_lambda :: [Int] -> [Int]
sqrs_lambda list = map (\x = x * x) list
...
sumsqr :: Int -> Int
sumsqr n = foldr (+) 0 list
where list = map (\x = x * x) [1..n]
//Start = sumsqr 5 // 55
//Start = sumsqr 0 // 0
//Start = sumsqr -4 // 0
...
f2 :: [Int] -> [Int]
f2 list = map ((*)2) new_list
where new_list = filter (\x = x > 0) list
//Start = f2 [1, 2, -2, 3, -4] // [2, 4, 6]




///// review filter, takeWhile
...
f3 :: [Int] -> [Int]
f3 list = map (\x = x / 2) new_list
where new_list = takeWhile ((<>)0) list
...
extract3 list = map (\x = (x !! 0, x !! ((length x) / 2), x !! (length x - 1))) list
...
// Sol 1: using tuples
//alternatingSum list = sum (map (\(x,y) = x * ((-1)^y)) new_list)
//where 	l = length list - 1
//		new_list = zip(list, [0..l])

// Sol 2: using list comprehension
alternatingSum list = sum [a * ((-1)^b) \\ a <- list & b <- [0..(length list - 1)]]
