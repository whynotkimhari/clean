...
//factor :: Int -> Int
factor n
| n < 0 = abort "N can not be less than zero"
| n == 0 = 1
= n * factor(n-1)
...
powerrec x n
| n < 0 = abort "Can not be less than zero"
| n == 0 = 1
= x * powerrec x (n - 1)
...
// Sol 1
sim x 
| y rem 2 == 0 = take y x == drop y x
| otherwise = take y x == drop (y + 1) x		// Because we need to pass the mid el
where y = (length x) / 2
...
// Way 1
//f5 y xs = [faux y (xs !! 0)] ++ (f5 y (drop 1 xs))

// Way 2
f5 y [x : xs] = [faux y x] ++ (f5 y xs)
...
not_five [] = []
not_five xs
| ((xs !! 0) == 5) = not_five (drop 1 xs)
= [xs !! 0] ++ not_five (drop 1 xs)
...
lasts :: [[Int]] -> [Int]
lasts [] = []
lasts [x : xs] = [(reverse x) !! 0 : lasts xs]
...
lastdel :: [[Int]] -> [[Int]]
lastdel [] = []
lastdel [x : xs] = [take ((length x) - 1) x : lastdel xs]
