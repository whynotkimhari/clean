...
abundant :: Int -> Bool 
abundant num = sum [i \\ i <- [1..(num-1)] | num rem i == 0] > num
...
strong_num :: Int -> Bool 
strong_num num = sum [prod [1..x] \\ x <- list] == num
...
filtered num list = [num^i \\ x <- list & i <- [0..] | x rem num == 0]
...
find x k list = flatten [filter ((==)(x+k)) list,filter ((==)(x-k)) list]

retrieve :: [Int] Int -> [(Int, Int)]
retrieve list num = sort (removeDup (flatten [ [(x,v) \\ v <- (find x num list)] \\ x <- list | (find x num list) <> [] ]))
...
find_max_index list 
| new_list <> [] = hd new_list
= -1
where new_list = [i \\ x <- list & i <- [0..] | x == maxList list]
...
median [] = -1.0 
median list 
| len rem 2 == 0 = toReal ( new_list !! mid + (new_list !! (mid - 1)) ) / 2.0
= toReal (new_list !! mid)
where 
	  len = length list
	  mid = len / 2
...
isPrime :: Int -> Bool
isPrime num = [x \\ x <- [1..num] | num rem x == 0] == [1,num]

find_prime_sum_pairs :: [Int] [Int] -> [(Int,Int)] 
find_prime_sum_pairs listA listB = [(a,b) \\ a <- listA, b <- listB | isPrime (a+b)]
...
inc3 :: [Int] -> [Int]
inc3 list = [g i x \\ i <- [1..] & x <- new_list]
where new_list = [0] ++ list
