...
countSumEvens [] = []
countSumEvens [x : xs] = [[length y, sum y] : countSumEvens xs]
where y = filter isEven x
...
countChar [] _ = []
countChar [c : cs] chr = [[length z, length c - length z] : countChar cs chr]
where y = map toLower c
	  f a b = a == b
	  z = filter (f chr) y
